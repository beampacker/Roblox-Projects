...
--
local default_color = Color3.fromRGB(172,194,217)
local Library = loadstring(
        game:HttpGet(
                "https://raw.githubusercontent.com/beampacker/Roblox-Projects/refs/heads/main/5len%20ui"
        )
)({
    cheatname = "5len",
    gamename = "5len",
...
do -- functions
    ...
    player_esp.functions.get_player = function()
        target_5len.target.entry = nil
        target_5len.target.part = nil
        target_5len.target.distance = Flags["Enable FOV"]
                ...
                for _, part_name in ipairs(bones_to_check) do
                    ...
                    if part then
                        ...
                        if on_screen then
                            ...
                            then
                                if
                                mouse_distance < target_5len.target.distance
                                        or (
                                        mouse_distance == target_5len.target.distance
                                                and distance_to_target
                                                < (target_5len.target.part and (
                                                target_5len.target.part.Position - cam_position
                                        ).Magnitude or math.huge)
                                )
                                then
                                    closest_entry = player
                                    closest_part = part
                                    target_5len.target.distance = mouse_distance
                                end
                            end
                        end
                    end
...
do -- menu
    do -- combat
        ...
        do
            ui.sections["rage_main"] = ui.tabs["rage"]:Section({ Name = "Player", Side = "Left", Size = 427 })
        ...
        do
            ...
            ui.tabs["Misc"]:Section({ Name = "Weapon Mods", Side = "Right", Size = 220 })
...
runService.RenderStepped:Connect(function(deltaTime)
    ...
    do -- visuals
        ...
        do
            ui.sections["esp"] = ui.tabs["visuals"]:Section({ Name = "ESP", Side = "Left", Size = 423 })
        ...
        do
            ui.sections["esp_colors"] = ui.tabs["visuals"]:Section({ Name = "ESP Colors", Side = "Right", Size = 257 })
        ...
        do
            ...
            ui.tabs["world"]:Section({ Name = "World Customisation", Side = "Right", Size = 287 })
        ...
        do
            ...
            do
                ui.sections["traps"] = ui.tabs["Misc"]:Section({ Name = "Misc Fun", Side = "Left", Size = 250 })
                ...
                ui.sections["traps"]:Toggle({ Name = "Always crit", Flag = "Always Crit", Default = false })
...
LPH_NO_VIRTUALIZE(function()
    ...
    local rayc = raycast_stuff.Raycast
        raycast_stuff.Raycast = function(
            ...
            if Flags["Always Crit"] and output[1] and output[1].Parent and output[1].Parent:IsA("Model") then
                local object_folder = output[1].Parent.Parent

                if
                object_folder
                        and (object_folder.Name == "Trees" or object_folder.Name == "Nodes")
                        and object_folder:IsA("Folder")
                then
                    local to_hit = output[1].Parent:FindFirstChild("NodeSpark")
                            or output[1].Parent:FindFirstChild("TreeX")

                    if to_hit and to_hit.PrimaryPart then
                        output[1] = to_hit.PrimaryPart
                    end
                end
            end
...
do -- loops
    ...
    run_service.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
        ...
        do -- update target
            if closest_part and (closest_part ~= target_5len.target.part) then
                target_5len.target.part = closest_part
                target_5len.target.entry = entry
                target_5len.target.distance = (closest_part.Position - camera.CFrame.Position).Magnitude
            end
        end

        highlight_player = target_5len.target.entry -- highlight

        do -- auto shoot
            if local_player.Character then
                if Flags["Enable Auto Shoot"] and target_5len and target_5len.target and target_5len.target.entry and target_5len.target.part and player_esp.functions.is_visible2(entry) then
                    mouse1press()
                    mouse1release()
                end
            end            
        ...
        do -- aimbot
            if (target_5len and target_5len.target and target_5len.target.entry and target_5len.target.part and target_5len.target.entry ~= local_player) then
                if (Flags["Enable Aimbot"] and Flags["Aimbot Keybind"]) then
                    local enemy_pos = target_5len.target.part.Position
